import { Client, TextChannel, GatewayIntentBits, ActivityType, REST, SlashCommandBuilder, Routes } from "discord.js";
    ...
    client.on("ready", async () => {
        if(config.discordSlashCommandInitialize){
            ...
            fsutil.writeFile(configPath,JSON.stringify(config, null, 2)).then(
                (onfulfilled)=>{ console.log("[discord-chat] updated config.json file.");},
                (onrejected)=>{ console.log(`[discord-chat] Error updating config.json file. ${onrejected}`)}
            )
        }
        ...
        client.on("messageCreate", (message: any) => {
            ...
            if(config.toInGameChatPrefix.returnDiscordName === "both"){
                discordName = message.member.displayName === message.member.user.username ? message.member.displayName : `${message.member.displayName} (${message.member.user.username})`;
            }else{
                discordName = config.toInGameChatPrefix.returnDiscordName === 'displayName' ? message.member.displayName : message.member.user.username;
            }
            tellAllRaw(
                `${config.toInGameChatPrefix.start}${discordName}${config.toInGameChatPrefix.end}${message.content}`
            );
        });
        if(config.discordSlashCommandEnabled) client.on('interactionCreate', async interaction => {
        ...
        process.on("message", (data) => {
            ...
            } else if (config.discordPresenceEnabled && data.event === "presence") {
...
function initiateSlashCommands(){
    ...
    const commands = []
    commands.push(new SlashCommandBuilder().setName(config.discordSlashCommandListName).setDescription(config.discordSlashCommandListText));
    commands.map(command => {command.toJSON()});
    rest.put(Routes.applicationGuildCommands(config.botId, config.guildId), { body: commands })
